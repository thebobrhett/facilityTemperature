...
strStart = cstr(year(now)) & "-" & cstr(month(now)) & "-" & cstr(day(now)) & " " & cstr(hour(now) - 1) & ":00"
...
do until objTransmitter.eof
  ...
  do until objElement.eof
    ...
    objCurrent.close
    strSQL = "delete from temp_current where transmitter = " & objTransmitter("id") & " and sensor = " & objElement("channel") & " and rec_time between '" & strStart & "' and '" & strEnd & "'"
    objCurrent.open strSQL, objConn
    objElement.movenext
  loop
  objElement.close
